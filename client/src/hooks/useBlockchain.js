...
export const useBlockchain = () => {
    ...
    useEffect(() => {
        const loadUsername = async () => {
            ...
            // Try to load username from blockchain with error handling
            try {
                ...
                // If blockchain query fails, assume no username exists
                setUsername(null);
            } finally {
                setIsLoadingUsername(false);
            }
    ...
    const fetchLeaderboard = useCallback(async () => {
        try {
            ...
            // Set empty leaderboard on error to prevent infinite retries
            setLeaderboard([]);
            setIsLoadingLeaderboard(false);
    ...
    const fetchPlayerBalance = useCallback(async () => {
        ...
        try {
            ...
            // Set balance to 0 on error to prevent infinite retries
            setPlayerBalance(0);
            setIsLoadingBalance(false);
            // Don't throw error to prevent component crash
            return 0;
    ...
    // Auto-fetch leaderboard on mount - removed to prevent infinite loops
    // Leaderboard will be fetched manually when needed
    
    // Auto-fetch balance when account changes
    useEffect(() => {
        if (account) {
            fetchPlayerBalance().catch(err => {
